...
export default function AssignTask({uuid,category}) {
    ...
    const all_assistants = useSelector(state => state.users);

    useEffect(() => {
        if (all_assistants && all_assistants.assistants && all_assistants.assistants.length < 1 && reload <= 2) {
            dispatch(getAllAssistants());
            setReload(prevReload => prevReload + 1);
        }
    }, [dispatch, reload]);
    const rows = all_assistants?.assistants?.content || [];
    ...
    const onSubmit = async (data) => {
        ...
        try {
            ...
            const response = await dispatch(assignConcern(data));
        ...
        <ThemeProvider theme={defaultTheme}>
            <Container component="main" maxWidth="xl">
                ...
                >
                    ...
                    <Stack>
                        {
                            succeed ? <Alert severity="success">Concern Assigned Successful.</Alert> : <></>
                        }
                        {
                            ...
                            <Controller
                                ...
                                render={({ field,fieldState }) =>(
                                    <TextField
                                        ...
                                        sx={{mt:3}}
                                    >
                                        <MenuItem >SELECT ASSISTANT</MenuItem>
                                        {Array.isArray(rows) && (
                                            rows.map((item,index) => (
